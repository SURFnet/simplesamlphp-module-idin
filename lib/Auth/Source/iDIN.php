...
class sspmod_idin_Auth_Source_iDIN extends SimpleSAML_Auth_Source {
    ...
    private function getUser() {
        ...
        if (!session_id()) {
            $this->debug('calling session_start()');
            session_start();
        }

        if (!isset($_SESSION['uid'])) {
            $this->debug('the user is not authenticated');
            return NULL;
        }

        $this->debug('creating attributes');
        $attributes = array(
    ...
    public function authenticate(&$state) {
         ...
         */
        $beginauth = SimpleSAML_Module::getModuleURL('idin/beginauth.php');
         ...
         */
         $this->debug('redirecting to beginauth');
        \SimpleSAML\Utils\HTTP::redirectTrustedURL($beginauth);
